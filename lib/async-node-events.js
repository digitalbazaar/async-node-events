...
// ## iterator
// returns an async/sync capable iterator function that, when called, will
// dispatch each listener appropriately. Asynchronous listeners are expected to
// call their provided iterator callback with an optional err param to indicate
// error states. Synchronous listeners may throw an error for the same purpose
// or return false to simply cancel.
...
function iterator(listeners, args, callback) {
  return function iterate(err) {
    ...
    // cancel further iteration when result is explicitly false
    ...
    if(listeners.length) {
      ...
      try {
        result = listener.apply(null, args.slice(0, -1));
      } catch(e) {
        return iterate(e, result);
      }

      // if `result` is a promise, wait to settle before iterating
...
// # EventEmitter
// constructor function, a drop-in replacement for ``EventEmitter`` almost
// every time (excepting when an event emits a function, then use ``emitSync``)
// that also allows the event listeners to cancel events if ``emit`` is
// provided with a callback.  Event listeners may be either synchronous like
// DOM events or asynchronous to better fit the Node.js style. (Because of
// that, making an event placed inside of a high performance loop is probably a
// bad idea because the event listeners can introduce an unpredictable delay.)
// The ``EventEmitter`` constructor function attaches three "please don't
// touch" properties to itself (or its inheriting object) to keep track of
// listeners and the maximum number of allowed listeners.
...
// ## emit
// takes and event name and a variable number of arguments. If the last
// argument is a function, it assumes it's a callback for the event and that
// this event is cancelable.  Listeners can only cancel events if their return
// value (or callback parameter) is truthy, else the event continues. Event
// cancelation immediately ends all processing of event listeners. The order of
// event listeners is roughly that of the order they were attached, but
// ``once`` listeners are given priority over ``on`` listeners (so its possible
// to do a one-time cancelation and not have any of the ``on`` listeners
// called).
EventEmitter.prototype.emit = function emit(eventName) {
  // Get the list of arguments for the listeners into an array
  const argsArray = [].slice.call(arguments, 1, arguments.length);
  // If the last argument is a function, assume its a callback for ``emit`` and
  // pop it off
  const callback = argsArray[argsArray.length - 1] instanceof Function ?
    argsArray[argsArray.length - 1] : undefined;
  ...
  // Create an array of listeners to call for this particular event, taking the
  // ``once`` listeners first and firing the removeListener event, then
  // attaching any ``on`` listeners. In both cases only if they exist.
  let listenersToCall = [];
  ...
  const iterate = iterator(listenersToCall, argsArray, callback);
...
// ## emitAsync
// In this case, forcing async means the same thing as the normal version of
// the function
EventEmitter.prototype.emitAsync = EventEmitter.prototype.emit;

// ## emitSync
// Unfortunately for synchronous emit, the logic is changed enough that simply
// wrapping ``sync`` (as done below with other methods) is not possible, but
// the overall pattern of getting the arguments, the listeners to call, and
// iterating through them is still the same. In this case, however, the last
// argument is always passed to the listeners and what the listeners return is
// only checked for canceling the listeners calling list, the result is never
// returned.
EventEmitter.prototype.emitSync = function emitSync(eventName) {
  ...
  const iterate = iterator(listenersToCall, argsArray);
  argsArray.push(iterate);
  iterate(null);
  return this;
};
...
EventEmitter.prototype.on = function on(eventName, listener) {
  ...
  const listenerCount =
    this._eventListeners[eventName].length +
    (this._oneTimeListeners[eventName] || []).length;
...
// ## onAsync
// Similarly for forced asynchronous functions, just attach a truthy ``async``
// property
...
// These methods are just synonyms for the ``on*`` methods, kept just to make
// sure ``EventEmitter`` should "just work" for the majority of use-cases of
// ``EventEmitter``.
...
// ## once
// The ``once`` method works very similar to the ``on`` method, except it
// operates on the ``_oneTimeListeners`` property, instead. This could probably
// be DRYed out a bit, but will only save a few lines of code and I don't
// expect this library to change much.
...
// ## onceSync
// Exactly like ``onSync``, but with a different method involved. Attempting to
// DRY this would produce a net *gain* in lines of code.
...
// ## removeListener
// Checks if the event has any listeners registered and then filters out the
// provided listener, firing the ``removeListener`` event if found. It must be
// a reference to the same function, not just have equal ``toString`` results.
...
// (if the event exists) and emits the ``removeListener`` event for each
// listener. If not called with an event name, it generates a list of events
// and recursively calls itself for each event.
...
// ## listenerCount
// Not sure why ``EventEmitter`` didn't make this a prototype method, but here
// you go. Given an ``EventEmitter`` instance and an event name, it returns the
// number of listeners registered on that event.
